...
Seurat is developed and maintained by the
[Satija lab](https://satijalab.org/seurat/authors.html)
...
**Exercise**: Use the `pbmc.sce` object to plot the expression level of MS4A1
across cell types using functionality from the 
[scater](https://bioconductor.org/packages/scater) package. 
(Hint: use the `plotExpression` function).

**Challenge**: Combine the marker gene expression plot with a reduced dimension
plot of all cells in PCA space, with cells colored by cell type. 
(Hint: use the `plotPCA` function).
...
**Exercise**: Use the `manno.seurat` object to produce a PCA plot with cells 
colored by timepoint (Hint: use the `DimPlot` function). 

**Challenge**: Combine the PCA plot with a ridge plot of ACTB expression, 
stratified by time point.
(Hint: use the `RidgePlot` function).
...
See Chapter 14: Dealing with big data in the OSCA book.
https://bioconductor.org/books/3.17/OSCA.advanced/dealing-with-big-data.html
We also include examples from the `BiocParallel` 
`r Biocpkg("BiocParallel", vignette = "Introduction_To_BiocParallel.html", label = "intro vignette")`. 
